...
st.title("Welcome to Youtube Comment Analyser")
st.markdown("An application that uses **sentiment analysis** on YouTube comments to judge the authenticity overall rating on the YouTube video")
mmm = st.text_input("Enter Video ID:")
...
if(mmm != None and mmm != ""):
    ...
    with open('comments.txt','r') as f:
        ...
        if(sentiment>0):
            st.markdown("Video has a positive response")
            st.markdown(sentiment)
        else:
            st.markdown("Video has a negative response")
            st.markdown(sentiment)


else:
    st.markdown("please enter a valid video ID")